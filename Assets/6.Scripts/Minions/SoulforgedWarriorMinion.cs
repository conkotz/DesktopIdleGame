...
public class SoulforgedWarriorMinion : MonoBehaviour
{
    ...
    [SerializeField] private float furiousSlamForwardOffset = 1.6f;
    ...
    private float _nextFuriousSlamTime;
        ...
        SkillsManager skillsManager = null)
    {
        ...
        _initialized = true;
        _nextFuriousSlamTime = Time.time + AbilityCombatPower.SoulforgedWarriorFuriousSlamCooldownSeconds;
    ...
    private static void ApplyPhysicalBuffToCharacter(CharacterStats targetStats)
    {
        ...
        {
            effectId = AbilityCombatPower.SoulforgedWarriorWarcryBuffEffectId,
    ...
    private void TauntNearbyEnemies(float range)
    {
        ...
        for (int i = 0; i < enemies.Length; i++)
        {
            ...
            enemy.ForceTauntToMinion(transform);
        }
    }

    private void TryPerformFuriousSlam()
    {
        if (!HasFuriousSlamEnhancement() || !combat || Time.time < _nextFuriousSlamTime)
            return;

        combat.CollectEnemiesInFrontArc(AbilityCombatPower.SoulforgedWarriorFuriousSlamRange, _aoeScratch);
        ...
        Vector3 slamCenter = transform.position + new Vector3(faceSign * furiousSlamForwardOffset, 0f, 0f);
        SoulforgedWarriorMinionVfx.PlayFuriousSlamShockwave(
            this,
            slamCenter,
            faceSign,
            AbilityCombatPower.SoulforgedWarriorFuriousSlamRange,
            furiousSlamShockwaveDuration);

        unit?.TriggerMeleeAttack();

        for (int i = 0; i < _aoeScratch.Count; i++)
            combat.TryApplyStrikeToEnemy(_aoeScratch[i], AbilityCombatPower.SoulforgedWarriorFuriousSlamDamageMultiplier);
    }
